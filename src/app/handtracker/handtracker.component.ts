...
export class HandtrackerComponent implements OnInit {
  ...
  */
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            if(closedhands===1 && pointing===1) this.detectedGesture = "One Hand Closed One Hand Pointing";
            if (openhands == 0 && closedhands == 0 && pointing == 0 && pinching == 0)
                this.detectedGesture = "None";
            this.emitPrediction(predictions);
  ...
  //TODO: fix logic to handle 2 hand predictions 
  emitPrediction(predictions:any){
    var bounds:number[]=[];
    if(this.detectedGesture==='None'){
      ...
      this.onPrediction.emit(new PredictionEvent(this.detectedGesture,bounds));
    }
